...
<script>
  ...
  const quickchatKeys = Array.from({ length: 10 }, (_, i) => `QuickChatText${i + 1}`);
  const quickchatHotkeys = Array.from({ length: 10 }, (_, i) => `QuickChatHotkey${i + 1}`);

  // Генерация ключей для OnStart
  const onstartKeys = Array.from({ length: 21 }, (_, i) => `StartChatString${i + 1}`);
